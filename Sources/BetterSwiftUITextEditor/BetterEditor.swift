...
public struct BetterEditor: View {
    ...
    /// Tracks the calculated line height for proper sizing (single line = minHeight)
    @State private var lineHeight: CGFloat = 0
    
    /// Tracks the measured height of the hidden Text view (true content size)
    ...
    /// The padding TextEditor adds around text content on iOS
    private var verticalPadding: CGFloat {
        #if os(iOS)
        return 17.66666666666667 // 8 top + 9.6667 bottom
        #else
        return 0
        #endif
    }
    ...
    public var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 4) {
            ZStack(alignment: .topLeading) {
                ...
                // MARK: Hidden measurement views
                
                // Measures the true content height by rendering the same text
                ...
                Text(text.isEmpty ? " " : text + " ")
                    ...
                    .padding(.bottom, {
                        #if os(iOS)
                        return 9.66666666666667 as CGFloat
                        #else
                        return 0 as CGFloat
                        #endif
                    }())
        ...
        // Update line count whenever totalHeight or lineHeight changes
        .onChange(of: totalHeight) { _, _ in
            updateLineCount()
        }
        .onChange(of: lineHeight) { _, _ in
            updateLineCount()
        }
    }
